(() => {
    ...
    const body = `<section class="workspace query-page">
      ...
      <div class="query-workbench">
        <section class="query-editor-panel">
          ...
          <div class="sql-editor"><div class="line-gutter" aria-hidden="true">${editorValue.split("\n").map((_, index) => `<span>${index + 1}</span>`).join("")}</div><textarea id="query-editor" spellcheck="false" autocomplete="off" aria-label="SQL query">${html(editorValue)}</textarea></div>
          <footer class="editor-foot"><span>UTF-8</span><span>SQL</span><span>Read-only guard active</span></footer>
        </section>
  ...
  async function runQuery() {
    ...
    if (editor) state.querySQL = editor.value;
  ...
  document.addEventListener("input", event => {
    if (event.target.matches("[data-connection-menu-search]")) {
      ...
      input?.focus();
      if (input) input.selectionStart = input.selectionEnd = position;
    } else if (event.target.id === "query-editor") {
      state.querySQL = event.target.value;
      const gutter = document.querySelector(".line-gutter");
      if (gutter) gutter.innerHTML = event.target.value.split("\n").map((_, index) => `<span>${index + 1}</span>`).join("");
    }
  });
  ...
  document.addEventListener("keydown", async event => {
    ...
    if (event.target.id === "query-editor" && (event.metaKey || event.ctrlKey) && event.key === "Enter") {
      event.preventDefault();
      await runQuery();
    }
    if (event.target.id === "query-editor" && event.key === "Tab") {
      event.preventDefault();
